...
def H_matrix(extended_D, encodedL):
    ...
    return G_hat * mrow(encodedL) * mcol(encodedL)
...
def H_kern_matrix(D, encodedL, Poly_RBF, K, c, d, gamma):
    ...
    return k_f_matrix * mrow(encodedL) * mcol(encodedL)
...
def SVM_wrapper(D, L, K_svm, C, priorT_b, idxTrain, idxTest, triplet, c=None, d=None, gamma=None, single_fold=True, show=True, kern=False, Poly_RBF=True, calibrate=False, saveCalibration=False, Gauss_flag=False, zscore=False):
    ...
    def SVM_scores(self, DTE, kern, PolyRBF, bounds_list, c=None, d=None, gamma=None):
        ...
        if kern:
           ...
           return kernel_SVM_scores(self.DTR, DTE, alpha_opt, self.z,
            ...
            # Linear SVM
            w_star = mcol(np.sum(mrow(alpha_opt) * mrow(self.z) * self.DTR, axis=1))
...
def K_fold_SVM(D, L, K, K_svm, C, priorT_b, app_triplet, PCA_m=None, seed=0, show=True, kern=False, Poly_RBF=True, c=None, d=None, gamma=None, calibrate=False, printStatus=False, returnScores=False,  Gauss_flag=False, zscore=False):
    if show:
        if kern:
            type_SVM = '{} Kernel'.format('Quadratic' if Poly_RBF else 'RBF')
        else:
    ...
    for j in range(K):
        ...
        if PCA_m is not None:
            ...
            PCA_P = PCA(DTR_PCA_fold, PCA_m)
            D_PCA = np.dot(PCA_P.T, D)
            scores = SVM_wrapper(D_PCA, L, K_svm, C, priorT_b, idxTrain, idxTest, app_triplet,
                c, d, gamma, single_fold=False, show=show, kern=kern, Poly_RBF=Poly_RBF, Gauss_flag=Gauss_flag, zscore=zscore)
        else:
